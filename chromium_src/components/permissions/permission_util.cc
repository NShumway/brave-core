...
#define PERMISSION_UTIL_PERMISSION_TYPE_TO_CONTENT_SETTINGS_TYPE \
  ...
  case PermissionType::BRAVE_CONTEXT_MENU:                       \
    return ContentSettingsType::BRAVE_CONTEXT_MENU;
...
bool PermissionUtil::GetPermissionType(ContentSettingsType type,
  ...
  if (type == ContentSettingsType::BRAVE_CONTEXT_MENU) {
    *out = PermissionType::BRAVE_CONTEXT_MENU;
    return true;
  }
...
PermissionType PermissionUtil::ContentSettingsTypeToPermissionType(
  ...
  switch (permission) {
    ...
    case ContentSettingsType::BRAVE_CONTEXT_MENU:
      return PermissionType::BRAVE_CONTEXT_MENU;
    default:
...
GURL PermissionUtil::GetCanonicalOrigin(ContentSettingsType permission,
  ...
  // Use requesting_origin for context menu permission so that iframe origins
  // are stored separately from their parent page's origin. This matches the
  // user's mental model of "this thing I right-clicked on" rather than the
  // top-level page.
  if (permission == ContentSettingsType::BRAVE_CONTEXT_MENU) {
    return requesting_origin;
  }
