...
GetNetworkTrafficAnnotationTagForFaviconFetch() {
  return net::DefineNetworkTrafficAnnotation("brave_rewards_favicon_fetcher",
                                             R"(
      semantics {
        sender:
...
void RewardsServiceImpl::OnReconcileComplete(
  ...
  for (auto& observer : observers_) {
    observer.OnReconcileComplete(this, result, contribution->contribution_id,
                                 contribution->amount, contribution->type,
...
void RewardsServiceImpl::LoadURL(mojom::UrlRequestPtr request,
  ...
  if (test_response_callback_) {
    ...
    test_response_callback_.Run(request->url, static_cast<int>(request->method),
                                &response_status_code, &test_response,
                                &test_headers);
  ...
  if (net_request->url.DomainIs("twitter.com")) {
    ...
    net_request->headers.SetHeader(net::HttpRequestHeaders::kUserAgent,
                                   "DESKTOP");
#endif
...
void RewardsServiceImpl::GetBalanceReport(const uint32_t month,
                                          const uint32_t year,
                                          GetBalanceReportCallback callback) {
...
void RewardsServiceImpl::GetPublisherInfo(const std::string& publisher_key,
...
void RewardsServiceImpl::GetRecurringTips(GetRecurringTipsCallback callback) {
  if (!Connected()) {
    return DeferCallback(FROM_HERE, std::move(callback),
                         std::vector<mojom::PublisherInfoPtr>());
  }
...
void RewardsServiceImpl::SetPublisherExclude(const std::string& publisher_key,
                                             bool exclude) {
  if (!Connected()) {
    return;
  }
...
void RewardsServiceImpl::LoadDiagnosticLog(const int num_lines,
...
void RewardsServiceImpl::RefreshPublisher(const std::string& publisher_key,
